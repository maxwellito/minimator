 ...
 */
export function share(title, url, file) {
    const basicShare = {
        url,
        title,
    };
 ...
 */
export function downloader(svgContent, fileName) {
    let blob = new Blob([svgContent], { type: 'octet/stream' }), url = window.URL.createObjectURL(blob);
